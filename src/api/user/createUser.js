...
exports.post = async (req, res, next) => {
  ...
  let user =
    ...
    '${name}', '${email}', '${tRetP}', '${phone}','${place}','${moment(dt_birthday).format('YYYY-MM-DD')}','${fk_id_role}',${active ? active : false},'${profile_pic ? profile_pic[0].data : ""
      }', false)`);

  if (fk_id_vehicle) {
    if (fk_id_vehicle[0] == 0) {
      let vehicles = await banco.query(`select id_vehicle from tb_vehicle where deleted = false`)
      await Promise.all(vehicles[0].map(async v => {
        await banco.query(
          `insert into tb_rel_user_vehicle (fk_id_user, fk_id_vehicle) values (${user[0].insertId}, ${v.id_vehicle})`
        );
